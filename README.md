...
Native vertebrae probes are still available, but they are now opt-in through
`ProbeConfig(enabled=True)`. That keeps the default report focused on overlap,
stability, and Separatix instead of fitting two separate probe systems.
...
- Treat `probe_accuracy` as an optional native quick-check column. It is blank unless you explicitly enable vertebrae's native probes.
